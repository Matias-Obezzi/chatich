...
class Grad implements GradType {
  x: number;
  y: number;
  z: number;
...
const Waves: React.FC<WavesProps> = ({
  ...
  useEffect(() => {
    ...
    function moved(point: Point, withCursor: boolean = true): { x: number; y: number } {
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute top-0 left-0 m-0 p-0 w-full h-full overflow-hidden ${className}`}
      style={{
        position: 'absolute',
        top: 0,
        left: 0,
